  ...
  <body class="animsition app-contacts" style="opacity:0">
    ...
    <div class="page" style="max-width: 100%; margin-top:0;margin-bottom:0">
      ...
      <div class="page-header">
        ...
        <a href="{{ route('classes.adicionar') }}" class="btn btn-success btn-lg"><i class="fa fa-plus" aria-hidden="true"></i> Adicionar Classe</a>
      </div>
         ...
         <div class="page-content-actions">
           ...
           <div class="row">
             <div class="col-xs-12 col-sm-3">
               <form action="{{ route('classes') }}" name="search" id="busca">
                 <div class="input-search">
                   <button type="submit" class="input-search-btn"><i class="icon md-search" aria-hidden="true"></i></button>
                   <input type="text" class="form-control" name="" placeholder="Buscar Classe">
                 </div>
               </form>
             </div>
              ...
              <div id="registros" class="panel-body">
                <table id="registros" class="table table-hover" data-toggle="table" data-sort-name="id" data-sort-order="asc" data-mobile-responsive="true">
                  ...
                  <tr>
                    ...
                    <th data-field="escola" data-sortable="true">Escola</th>
                    <th data-field="ano" data-sortable="true">Ano</th>
                    <th data-field="classe" data-sortable="true">Classe</th>
                    <th class="text-center" data-field="qtde" data-sortable="true">Qtde Alunos</th>
                  ...
                  @foreach($registros as $registro)
                    <tr id="registro-{{ $registro->id }}">
                      ...
                      <td class="text-center cell-30">{{ $registro->id }}</td>
                      <td>{{ $registro->school->name }} </td>
                      <td>{{ $registro->year->title }}</td>
                      <td>{{ $registro->name }}</td>
                      <td class="text-center">{{ $registro->number }}</td>
                      <td>
                        <a title="Editar {!! $registro->name !!}" href="{{ route('classes.editar', $registro->id) }}" class="btn btn-success"><i class="icon fa fa-pencil"></i> Editar</a>
      ...
      <div class="modal-dialog modal-center text-center modal-sm">
        <div class="modal-content">
          ...
          <div class="modal-body" style="padding-top:0">
            ...
            <p style="margin-bottom:0">Tem certeza que deseja excluir a classe <b id="nameData"></b>?</p>
          </div>
  ...
  <script type="text/javascript">
    jQuery(document).ready(function ($) {
      ...
      function deleteData (id) {
        $.ajax({
          dataType: 'json',
          url: '{{ URL::asset("classes/deletar") }}/'+id,
          type: 'GET',
          success: function (json) {
            if (json.success) {
              ...
              $('#deleteData').modal('hide');
              toastr.warning('Ops, houve um erro ao excluir a classe.');
            }
          },
          error: function () {
            $('#deleteData').modal('hide');
