@extends('layout.page')
@section('title', 'Catetorias Profissionais')
  ...
  <body class="animsition app-clinicas app-contacts page-aside-left" style="opacity:0">
    ...
    <div class="page bg-white" style="max-width: 100%; margin-top:0;margin-bottom:0">
      ...
      <div class="page-main">
        ...
        <div style="overflow: hidden;" class="page-header">
          <h1 style="float: left" class="page-title">Categorias Profissionais</h1>

          <div class="pull-sm-right">
              <form action="{{ route('categoriasprofissional') }}" name="search" id="busca">
                <div class="input-search input-search-dark">
                  <i class="input-search-icon md-search" aria-hidden="true"></i>
                  <input type="text" class="form-control" name="query" value="{{ isset($_GET['query']) ? $_GET['query'] : '' }}" placeholder="Buscar Categoria Profissional">
                </div>
              </form>
            </div>
        </div>
              ...
              <table id="registros" class="table table-striped" data-toggle="table" data-sort-name="id" data-sort-order="asc" data-mobile-responsive="true">
                <thead>
                  <tr>
                    ...
                    <th data-field="tipo" data-sortable="true">Tipo de Pessoa</th>
                    @can('edit_categoriaprofissional', 'delete_categoriaprofissional' )
                ...
                <tbody>
                  @foreach($registros as $registro)
                    <tr id="registro-{{ $registro->id }}">
                      ...
                      <td>{{ $registro->nome }}</td>
                      <td>@if(isset($registro->tipo_pessoa) and $registro->tipo_pessoa == 'f') Física @else Juridica @endif</td>

                      @can('edit_categoriaprofissional', 'delete_categoriaprofissional' )
                        <td>
                          @can('edit_categoriaprofissional')
                            <a style="text-decoration:none" title="Editar {!! $registro->nome !!}" href="{{ route('categoriasprofissional.editar', $registro->id) }}" class="btn btn-success"><i class="icon fa fa-pencil"></i> Editar</a>
                          @endcan
                          @can('delete_categoriaprofissional')
      ...
      <div class="modal-dialog modal-center text-center modal-sm">
        <div class="modal-content">
          ...
          <div class="modal-body" style="padding-top:0">
            ...
            <p style="margin-bottom:0">Tem certeza que deseja excluir a Categoria Profissional <b id="nameData"></b>?</p>
          </div>
    ...
    @can('add_categoriaprofissional')
      <div class="site-action" data-plugin="actionBtn">
        ...
        <a href="{{ route('categoriasprofissional.adicionar') }}" class="btn-raised btn btn-success btn-floating">
  ...
  <script type="text/javascript">
    jQuery(document).ready(function ($) {
      ...
      function deleteData (id) {
        $.ajax({
          dataType: 'json',
          url: '{{ URL::asset("categorias-profissional/deletar") }}/'+id,
          type: 'GET',
          success: function (json) {
            if (json.success) {
              ...
              $('#deleteData').modal('hide');
              toastr.warning('Ops, houve um erro ao excluir a Categoria Profissional.');
            }
          },
          error: function () {
            $('#deleteData').modal('hide');
