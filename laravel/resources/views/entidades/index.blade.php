  ...
  <body class="animsition app-clinicas app-contacts page-aside-left" style="opacity:0">
                ...
                <div class="form-group">
                 ...
                 <div class="form-group">
                   ...
                   @if((isset($_GET['estado']) and !empty($_GET['estado'])) or (isset($_GET['cidade']) and !empty($_GET['cidade'])) )
                    <a style="margin-left:10px;" href="{{ route('entidades') }}" title="Limpar" class="btn btn-default">Limpar</a>
                   @endif
                 </div>
            </form>
            <div class="pull-sm-right">
              <form action="{{ route('entidades') }}" name="search" id="busca">
                <div class="input-search input-search-dark">
                  <i class="input-search-icon md-search" aria-hidden="true"></i>
                  <input type="text" class="form-control" name="query" value="{{ isset($_GET['query']) ? $_GET['query'] : '' }}" placeholder="Buscar Entidade">
                </div>
              </form>
            </div>
          </div>
              ...
              <table id="registros" class="table table-striped" data-toggle="table" data-sort-name="id" data-sort-order="asc" data-mobile-responsive="true">
                <thead>
                  <tr>
                    ...
                    <th data-field="sigla" data-sortable="true">Sigla</th>
                    <th data-field="nome" data-sortable="false">Nome</th>
                    <th data-field="classificacao" data-sortable="true">Classificação</th>
                    @can('edit_entidade', 'delete_entidade' )
                ...
                <tbody>
                  @foreach($registros as $registro)
                    <tr id="registro-{{ $registro->id }}">
                      ...
                      <td>{{ $registro->classificacao->nome }}</td>
                      
                      @can('edit_entidade', 'delete_entidade' )
                        <td>
                          @can('edit_entidade')
                            <a style="text-decoration:none" title="Editar {!! $registro->nome !!}" href="{{ route('entidades.editar', $registro->id) }}" class="btn btn-success"><i class="icon fa fa-pencil"></i> Editar</a>
                          @endcan
                          @can('delete_entidade')
      ...
      <div class="modal-dialog modal-center text-center modal-sm">
        <div class="modal-content">
          ...
          <div class="modal-body" style="padding-top:0">
            ...
            <p style="margin-bottom:0">Tem certeza que deseja excluir a entidade <b id="nameData"></b>?</p>
          </div>
    ...
    @can('add_clinica')
      <div class="site-action" data-plugin="actionBtn">
        ...
        <a href="{{ route('entidades.adicionar') }}" class="btn-raised btn btn-success btn-floating">
  ...
  <script type="text/javascript">
    jQuery(document).ready(function ($) {
      ...
      function deleteData (id) {
        $.ajax({
          dataType: 'json',
          url: '{{ URL::asset("clinicas/deletar") }}/'+id,
          type: 'GET',
